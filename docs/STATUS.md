...
- Thumbnail contract `auto-thumbnail-v5` is live. Historical production outputs still bootstrap the persistent novelty registry, while text styling now enforces a white main fill plus a maximum of 1-2 selected hook words in one contrast-selected accent from yellow/red/blue/green. Full-headline accent coloring is no longer valid production output. Top/middle/bottom placement remains selected only from face/protected-subject/text-safe candidates, and QC/metadata now persist `accent_words`, `accent_color`, and the `white_main_selective_accent_v1` color contract.
...
- Production variation shortlist is `orion`, `luna`, `ara`, `lux`, `altair`; all 28 documented built-in voice IDs remain exposed by `/api/voices`.
...
## 2026-09-05 Runs 14-18 durability acceptance

- The Martial Genius Who Remembers Everything was exercised sequentially across chapters 13-15, 10-12, 7-9, 4-6, and 1-3. All five final run states are PASS, every final QC is fail-free, publication count is zero, and later runs were not started until the preceding run had reached PASS.
- Run 14 exposed a deterministic recovery bug in `production_visual_selection_insufficient`: corrective synthesis counted section-safe panels as if they were also generic production-safe. The recovery helper now satisfies section-safe minimums, generic-safe minimums, and the existing global 18-unique generic-safe requirement independently. No threshold was reduced. The repaired Run 14 resumed from observation/frameability cache and reached RECONCILED with zero repeated observation provider calls.
- Run 15 then completed cleanly on the new logic with analysis attempt 1 and production attempt 1, demonstrating that the Run 14 recovery change did not regress a new corpus.
- Run 16 hit a real transient `vision_provider_request_failed` on analysis attempt 1; the blessed launcher retried automatically and reached RECONCILED on attempt 2 while reusing cached visual work. Production completed on attempt 1.
- Run 17 hit a real transient `vision_response_invalid` on analysis attempt 1; attempt 2 recovered automatically with observation/frameability cache reuse and production completed on attempt 1.
