...
## 2026-08-23 - Source-level streaming callback and prepared/filter audit

- Published parent for this source/test/docs checkpoint: `e0e6db5b39e9033b5d0a77bfab5a15afb2d3df05`. The implementation adds an opt-in `on_reconciled(group, result)` callback to `strip_segmentation.reconcile_sources`; streaming `prepare_project_panels` builds one canonical local coverage map, admits the completed source group's exact regions through `panel-admission-v1`, and calls the existing single writer before later source groups finish. Default non-stream callers preserve reconciliation-first behavior and error classification.
- The admission funnel is explicit and local: raw input images | ingest assets | candidate regions | canonical regions | admitted vision panels. Every transition has count, elapsed time, stable reason code; every decision retains source asset/checksum, original bounds, candidate IDs, detector/version, reproducible metrics, and ledger hash. Protected/dialogue/ambiguous material still fails closed to `NEEDS_REVIEW`; no gate was relaxed and no provider decides basic filtering.
- GREEN evidence: `tests/test_cloud_multimodal_mass_production.py` 170 passed / 5 existing Pillow deprecation warnings; `tests/test_strip_segmentation.py tests/test_segmentation.py tests/test_segmentation_reconciliation.py` 47 passed / 8 existing Pillow deprecation warnings; Ruff, compileall, and `git diff --check` passed. The 13 `tests/test_pipeline.py` fixture failures remain unchanged and outside a full-suite GREEN claim. This code/test slice consumed no provider or TTS request.
- Clean v7 normal-entrypoint probe `/data/data/p0-aws-acceptance/video1-clean-proof-v7` was stopped safely after 4m13s when no first visual dispatch or visual checkpoint had appeared; 12 segmentation-review reports were observed at stop and 13 were present in the preserved read-only namespace afterward. No story map, narration, TTS, render, or QC artifact exists from v7.
- Read-only database audit: current `SCRIPT_DRAFT` analysis `54fc779ba2334d55a46f815fa56ccd6c` has 701 distinct canonical rows, source-order domain 0..702, and exact gaps 303/306; source assets=646. Two older analyses have 280 rows and are stale. This is the evidence behind the observed prepared=703/filter=701 arithmetic, not a semantic reason-code ledger; do not claim those two rows were gutter/blank/deduped until a fresh funnel run records the exact local decision.
- Next gate: run a new 40-80 panel namespace through the source callback and report `raw | ingest | candidate | canonical | admitted | rejected | deduped | merged | needs-review`, first-dispatch/overlap timing, terminal N/N, and provider error taxonomy. Preserve v6/v7 read-only; do not start story, narration, TTS, or render from a partial visual subset.

## 2026-08-23 - Callback subset blocker: strict segmentation remains unresolved

- Fresh callback subset `video1-stream-source-callback-v1`: selected 40 canonical regions from 37 assets / 7 complete source groups; two provisional visual chunks were emitted before `segmentation.ambiguous_boundary` invalidated the prepare phase. No terminal N/N, final funnel ledger, story, narration, TTS, render, or QC evidence is claimed.
- Fresh non-overlapping local-only subset `video1-stream-source-callback-local-v1`: selected 41 canonical regions from 39 assets / 8 complete groups; first visual dispatch was 9.876s from start and before preparation returned at 15.745s, with zero boundary-provider requests and one provisional visual chunk. It then failed closed at `segmentation.ambiguous_boundary`; the partial runtime remains read-only and is not proof.
- Offline sanitized replay of source family `129__010` proves the blocker is a genuine boundary contract result: accepted candidate positions were 2426/3866/5229, while deterministic ideals were 1600/3200/4800/6400/8000/9600/11200/12800/14400. The configured proximity and partition checks therefore rejected the cut set. No gate or threshold is relaxed; no review-only override is used for production proof.
- Next action is a narrow RED/GREEN segmentation contract fix or a new source subset whose provider/local evidence satisfies the existing geometry contract. Do not start the 701-panel clean run, story/narration, TTS, or render from either partial namespace.

## 2026-08-23 - Nonuniform segmentation admission fix (unpublished)

- Root cause: `reconcile_strip` greedily required every accepted boundary to be within a fixed 20%-of-frame radius of each equal-height ideal. A provider-confirmed nonuniform cut could therefore be rejected before partition geometry was evaluated. This was a local false rejection, not evidence to relax protected/balloon/lineage gates.
- Fix: `SEGMENTATION_VERSION` is `color-agnostic-strip-reconciliation-v2`; a bounded deterministic partition selector consumes only provider-accepted candidates (or local high-confidence candidates), prefers the target segment count, then geometry deviation, confidence, and source position. Hard checks require minimum span, a maximum of two target frame-heights, enough cuts to cover the full source, unique ordered cuts, and complete coverage. Sparse/oversized candidates remain `segmentation.ambiguous_boundary`.
