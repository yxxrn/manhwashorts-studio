...
- Final-video channel watermark is a first-class optional project setting. Default is OFF. When enabled it must use the configured free-form text, lower-center placement, translucent white styling, and the bundled Caacupe One watermark font with synthetic bold (never the subtitle font), and be fused into the same final encode pass as subtitles. The watermark font checksum is part of render identity. Watermark enable/text are part of render identity: changing either must invalidate reuse of the prior final artifact. Never apply the watermark to review/preview renders.
