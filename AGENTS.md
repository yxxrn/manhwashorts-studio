...
- Production Grok TTS uses `grok-voice-latest` with an explicit provider `voice_id`; never rely on a descriptive pseudo-voice label that is not sent to the provider. New English projects default to `ara`; legacy `the-explainer-american` resolves to that real voice for compatibility. Voice variation changes `voice_id`, not the model, and one render must keep one immutable voice profile.
