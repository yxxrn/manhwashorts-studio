...
from app.services import comic_render, comic_thumbnail, visual_scoring
...
def _topic_thumbnail_config(script: object) -> dict[str, object]:
    """Compatibility adapter for the centralized Comic Lore profile."""
    return comic_lore_profile_from_script(script).thumbnail_binding(clean_headline=_clean_headline)
...
_thumbnail_strict_yellow_text_cleanup = comic_thumbnail.cleanup_yellow_dialogue
...
def _clean_scene_image(scene: object, source_path: Path, *, comic_text_cleanup: bool = False) -> bytes:
    ...
    if comic_text_cleanup:
        ...
        try:
            ...
            comic_render.cleanup_comic_frame(temporary_path)
...
def _comic_dual_palette(image: Image.Image, placement: str):
    return comic_thumbnail.dual_contrast_palette(image, placement, _THUMBNAIL_ACCENT_COLORS)
