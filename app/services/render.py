...
class RenderRequest:
    ...
    watermark_text: str = ""
    adaptive_karaoke_contrast: bool = False
...
COMIC_TEXT_CLEANUP_VERSION = comic_render.COMIC_TEXT_CLEANUP_VERSION
ADAPTIVE_KARAOKE_CONTRAST_VERSION = comic_render.ADAPTIVE_KARAOKE_CONTRAST_VERSION


def _adaptive_karaoke_color_for_video(image_paths: Sequence[Path]) -> dict[str, object]:
    try:
        return comic_render.select_locked_karaoke_color(image_paths)
    except comic_render.AdaptiveKaraokeError as exc:
        raise RenderError(str(exc), code=exc.code) from exc


_comic_text_cleanup_heuristic_v1 = comic_render.cleanup_comic_frame_heuristic
_comic_text_cleanup_frame = comic_render.cleanup_comic_frame


def _karaoke_color_for_time(spans: Sequence[Mapping[str, object]], timestamp: float) -> str:
    for span in spans:
        try:
            if float(span.get("start_time", -1.0)) <= timestamp < float(span.get("end_time", -1.0)):
                color = str(span.get("ass_color", "") or "").upper()
...
def build_sentence_karaoke_ass(
    ...
    *,
    ...
    active_color_spans: Sequence[Mapping[str, object]] = (),
) -> str:
            ...
            for line in wrapped:
                ...
                for display_word in line.split():
                    if word_index == active_index:
                        active_color = _karaoke_color_for_time(active_color_spans, word.start_time)
                        tag = f"\\c&H00{active_color}&\\fscx{active_percent}\\fscy{active_percent}"
                    else:
...
def render_video(request: RenderRequest, progress=None) -> RenderResult:
    ...
    cleanup_telemetry: list[dict[str, object]] = []
    for i, scene in enumerate(request.scenes):
        ...
        if request.comic_text_cleanup and not request.preview:
            cleanup_entry = dict(_comic_text_cleanup_frame(prepared))
            cleanup_entry["scene_index"] = i
            cleanup_telemetry.append(cleanup_entry)
    ...
    if request.sentence_groups:
        ...
        ass_text = build_sentence_karaoke_ass(
            ...
            font_height_ratio=subtitle_karaoke.CAPTION_FONT_HEIGHT_RATIO,
            safe_margin_px=subtitle_karaoke.CAPTION_SAFE_MARGIN_PX,
            active_color_spans=adaptive_color_spans,
        )
    ...
    if request.silent_reference_review:
        ...
        manifest = {
            ...
            "comic_render_cleanup": {
                "enabled": bool(request.comic_text_cleanup),
                "version": COMIC_TEXT_CLEANUP_VERSION,
                "processed_scenes": len(cleanup_telemetry),
                "applied_scenes": sum(1 for entry in cleanup_telemetry if entry.get("applied") is True),
                "wall_s": round(sum(float(entry.get("wall_s", 0.0) or 0.0) for entry in cleanup_telemetry), 6),
                "entries": cleanup_telemetry,
            },
